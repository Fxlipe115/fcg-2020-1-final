...
void ObjectModel::draw(GpuProgram* gpuProgram) {
  for(std::string meshName : this->meshNames) {
    this->virtualScene->drawObject(meshName.c_str(), gpuProgram);
  }
}


void ObjectModel::printObjectModelInfo()
{
  const tinyobj::attrib_t                & attrib    = this->attrib;

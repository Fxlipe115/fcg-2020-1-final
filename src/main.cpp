...
int main(int argc, char* argv[])
{
    ...
    spheremodel.buildTrianglesAndAddToVirtualScene(g_VirtualScene);

    ObjectModel bunnymodel("./data/bunny.obj");
    bunnymodel.computeNormals();
    bunnymodel.buildTrianglesAndAddToVirtualScene(g_VirtualScene);

    ObjectModel planemodel("./data/plane.obj");
    planemodel.computeNormals();
    planemodel.buildTrianglesAndAddToVirtualScene(g_VirtualScene);

    if ( argc > 1 )
    {
        ObjectModel model(argv[1]);
        model.buildTrianglesAndAddToVirtualScene(g_VirtualScene);
    }
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        g_VirtualScene.drawObject("sphere");
        ...
        g_VirtualScene.drawObject("bunny");

        model = Matrix_Translate(0.0f,-1.0f,0.0f)
              * Matrix_Scale(2.0f,1.0f,2.0f);
        glUniformMatrix4fv(model_uniform, 1 , GL_FALSE , glm::value_ptr(model));
        glUniform1i(object_id_uniform, PLANE);
        g_VirtualScene.drawObject("plane");
